...
import UploadMoto from "./UploadMoto";
...
const ModalCreateMoto = ({isVisible, setVisible, setDataSource, dataSource}) => {
  ...
  return (
    <Modal
      ...
      destroyOnClose={true}
      title={t("modal.createMotorcycle")}
      centered={true}
      open={isVisible}
      onCancel={setVisible}
      onOk={handlePetition}
      ...
      <Form.Item
        label="Color:"
        name="color"
        rules={[{ required: true, message: t("messageError.emptyColor") }]}
      >
        <Select
          onSelect={(value) => setColor(value)}
          style={{ marginBottom: margin, width: 100 }}
          placeholder="Color"
        >
          <Select.Option value="Rojo">
            {t("mainContent.table.colors.red")}
          </Select.Option>
          <Select.Option value="Azul">
            {t("mainContent.table.colors.blue")}
          </Select.Option>
          <Select.Option value="Negro">
            {t("mainContent.table.colors.black")}
          </Select.Option>
          <Select.Option value="Blanco">
            {t("mainContent.table.colors.white")}
          </Select.Option>
          <Select.Option value="Rojo-Negro">
            {t("mainContent.table.colors.red-black")}
          </Select.Option>
          <Select.Option value="Azul-Negro">
            {t("mainContent.table.colors.blue-black")}
          </Select.Option>
          <Select.Option value="Rojo-Blanco">
            {t("mainContent.table.colors.red-white")}
          </Select.Option>
          <Select.Option value="Blanco-Negro">
            {t("mainContent.table.colors.white-black")}
          </Select.Option>
        </Select>
      </Form.Item>
      ...
      >
        <Select
          ...
          placeholder={t("mainContent.table.situation")}
        >
          {situationData.map((item, i) => (
            <Select.Option key={i} value={item.situacion}>
              {item.situacion}
            </Select.Option>
          ))}
        </Select>
